...
│
├── src/main/java/com.controleFinanceiro        # Código-fonte Java
│   │
│   ├── Main.java                               # Ponto de entrada da aplicação
│   │
│   ├── db/
│   │   ├── ConnectionFactory.java                     # Singleton de conexão JDBC
│   │
...
- [Maven](https://maven.apache.org/) (já incluso no IntelliJ)
...
4. Navegue até `sql/controle_financeiro.sql`
...
SOURCE caminho/do/projeto/sql/controle_financeiro.sql;
```
...
Abra o arquivo `src/main/java/com/controleFinanceiro/db/ConnectionFactory.java` e ajuste com suas credenciais:

```java
private final String usuario = "root";   // seu usuário MySQL
private final String senha   = "root";   // sua senha MySQL
...
No IntelliJ, clique no ícone 🔄 **Reload Maven** no painel do Maven (canto direito) para baixar as dependências automaticamente.
...
Use as credenciais abaixo para testar o sistema localmente após rodar o script `controle_financeiro_seed.sql`.

| Perfil | Nome | E-mail | Senha | Acesso |
|---|---|---|---|---|
| 🔑 Administrador | Administrador | admin@teste.com | 123 | Total — gerencia usuários, categorias e dicas |
| 👤 Usuário Comum | Usuario Teste | usuario@teste.com | 123 | Restrito — transações, metas e orçamentos próprios |

> ⚠️ **Atenção:** estas credenciais são exclusivas para ambiente de desenvolvimento e testes. Não utilize em produção.
